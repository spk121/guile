...
#define FUNC_NAME s_scm_sigaction_for_thread
{
  ...
  if (SCM_UNBNDP (handler))
    ...
    {
      long_t handler_int = scm_to_long_t (handler);

      if (handler_int == (long_t) SIG_DFL || handler_int == (long_t) SIG_IGN)
	{
#ifdef HAVE_SIGACTION
  ...
  */
  ...
  if (old_action.sa_handler == SIG_DFL || old_action.sa_handler == SIG_IGN)
    old_handler = scm_from_long_t (old_action.sa_handler);
  ...
  if (old_chandler == SIG_DFL || old_chandler == SIG_IGN)
    old_handler = scm_from_long_t ((long_t) old_chandler);
...
scm_init_scmsigs ()
{
  ...
  scm_c_define ("SIG_IGN", scm_from_long_t ((long_t) SIG_IGN));
  scm_c_define ("SIG_DFL", scm_from_long_t ((long_t) SIG_DFL));
#ifdef SA_NOCLDSTOP
