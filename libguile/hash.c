...
static scm_t_hash scm_raw_ihashq (scm_t_bits key);
static scm_t_hash scm_raw_ihash (SCM obj, size_t depth);
...
scm_raw_ihash (SCM obj, size_t depth)
{
  ...
  switch (SCM_TYP7(obj))
	...
	size_t len = SCM_SIMPLE_VECTOR_LENGTH (obj);
        size_t i = depth / 2;
        scm_t_hash h = scm_raw_ihashq (SCM_CELL_WORD_0 (obj));
        if (len)
          while (i--)
...
scm_t_hash
scm_ihashq (SCM obj, scm_t_hash n)
{
...
{
  scm_t_hash sz = scm_to_unsigned_integer (size, 1, UINTPTR_MAX);
...
scm_t_hash
scm_ihashv (SCM obj, scm_t_hash n)
{
  if (SCM_NUMP(obj))
...
scm_t_hash
scm_ihash (SCM obj, scm_t_hash n)
{
  return scm_raw_ihash (obj, 10) % n;
}
